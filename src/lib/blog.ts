...
/**
 * Content desks — collapse raw category strings into the workshop
 * disciplines this blog covers. Drives the marker colour (--v-*) on rows and
 * kickers so Gear / Craft / Field / Care read differently at a glance.
 * (Vertical keys kept as dev/finance/culture/health for token reuse; only the
 * labels + regex map are niche-specific.)
 */
...
/** Map a raw category string → one of the workshop desks. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(care|upkeep|maintenance|repair|clean|storage|safety)/.test(c)) return 'health'
  if (/(craft|build|project|woodwork|make|diy|technique|process)/.test(c)) return 'finance'
  if (/(field|photography|travel|outdoor|culture|community|history)/.test(c)) return 'culture'
  if (/(gear|keyboard|tool|kit|hardware|review|buying|equipment)/.test(c)) return 'dev'
  return 'personal'
}
